...
import MapInfo3 from '../mapinfo2/MapInfo3';
...
const Map4 = () => {
  ...
  useEffect(() => {
    ...
    const svg = d4.select('#heatmapSVG0');
    ...
    const hexagons = svg.append('g')
        ...
        .on('mouseover', function (event, d, i) {
          ...
          const tooltip0 = document.querySelector('.tooltip22');
          tooltip0.innerHTML = `ID: ${d4.select(this).attr('id')}`;
          tooltip0.style.display = 'block';
          tooltip0.style.left = event.pageX + 'px';
          tooltip0.style.top = event.pageY + 'px';
        })
        .on('mouseout', function (event) {
          // Hide tooltip when mouseout
          document.querySelector('.tooltip22').style.display = 'none';
        });
      ...
      <div className='map3'>
        <div className="mapContainer22">
          <div className="background22">
            <svg id="heatmapSVG0" className="heatmapSVG22" width="330" height="223"></svg></div>
        </div>
        <div className="tooltip22"></div>
        <MapInfo3 />
      </div>
  )
}
